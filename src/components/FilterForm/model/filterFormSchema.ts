...
const VALIDATION_FIELD_ACT_MSG = `Поле '№ заказа' должно содержать от 5 до 8 символов`;
...
export const filterFormSchema = Yup.object().shape({
  ...
  act: Yup.string().min(2, VALIDATION_FIELD_ACT_MSG).max(8, VALIDATION_FIELD_ACT_MSG)
});
